-- Edição segura de rascunhos e agendamentos antes da primeira tentativa de publicação.
-- Espelha a migration 026 aplicada no Supabase oficial.

create or replace function public.update_post_plan(
  p_post_id uuid,
...
begin
  ...
  if exists (
    ...
    where pt.post_id = p_post_id
      ...
      and pt.state not in ('DRAFT','SCHEDULED','CANCELLED')
  ) or exists (
    select 1
    from public.publication_attempts pa
    join public.post_targets pt on pt.id = pa.post_target_id
    where pt.post_id = p_post_id and pt.organization_id = v_org_id
  ) then
    raise exception 'post can no longer be edited safely' using errcode = '55000';
  end if;

  select ptm.media_asset_id into v_media_asset_id
  ...
  delete from public.post_targets
   where post_id = p_post_id and organization_id = v_org_id;

  update public.posts
     set internal_title = coalesce(nullif(btrim(p_internal_title), ''), 'Nova publicação'),
         base_caption = nullif(p_base_caption, ''),
         updated_at = now()
  ...
  loop
    ...
    if v_requested_action not in ('draft','schedule','publish_now') then raise exception 'invalid requested action' using errcode = '22023'; end if;
    if v_retention not in ('delete','library') then raise exception 'invalid retention mode' using errcode = '22023'; end if;
    if v_intent not in ('AUTO','SHORT_FORM','LONG_FORM','IMAGE','CAROUSEL','TEXT') then raise exception 'invalid content intent' using errcode = '22023'; end if;
    ...
    if v_connection_id is null or v_provider = '' then raise exception 'target connection/provider is required' using errcode = '22023'; end if;
    ...
    if v_requested_action = 'publish_now' then
      v_scheduled_at := now(); v_state := 'SCHEDULED';
    elsif v_requested_action = 'schedule' then
      v_scheduled_at := coalesce(nullif(v_target->>'scheduled_at', '')::timestamptz, now()); v_state := 'SCHEDULED';
    else
      v_scheduled_at := coalesce(nullif(v_target->>'scheduled_at', '')::timestamptz, now()); v_state := 'DRAFT';
    end if;
    ...
    insert into public.post_targets (
      ...
      v_org_id, p_post_id, v_connection_id, v_provider, 'AUTO',
      ...
      jsonb_build_object(
        ...
        'media_pipeline',
          case when v_provider = 'youtube' and v_intent = 'LONG_FORM' and coalesce(v_file_size, 0) > 2147483648
               then 'LARGE_FILE_TEMP' else 'STANDARD' end,
        'source', 'composer_edit'
      ),
      'post:' || p_post_id::text || ':' || v_connection_id::text || ':' || v_intent,
      v_caption, v_title
  ...
  if v_media_asset_id is not null then
    ...
    select v_org_id, pt.id, v_media_asset_id, 0, 'PRIMARY'
...
revoke all on function public.update_post_plan(uuid,text,text,jsonb) from public;
grant execute on function public.update_post_plan(uuid,text,text,jsonb) to authenticated;
