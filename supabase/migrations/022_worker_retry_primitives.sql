-- Primitivas server-only do worker de publicação.
-- Espelha a migration 022 já aplicada no projeto Supabase oficial.

create or replace function public.worker_claim_publication_jobs(
...
begin
  ...
  for v_job in
    select j.id, j.organization_id, j.post_target_id, j.attempts,
           j.max_attempts, j.payload, pt.provider,
           pt.content_intent_override, pt.scheduled_at
...
revoke all on function public.worker_claim_publication_jobs(integer,integer) from public, anon, authenticated;
grant execute on function public.worker_claim_publication_jobs(integer,integer) to service_role;

create or replace function public.worker_finish_publication_job(
  p_job_id uuid,
  p_outcome text,
...
begin
  ...
  if v_job.status <> 'RUNNING' then
    raise exception 'job is not running' using errcode = '22023';
  end if;
  ...
  if p_outcome not in ('SUCCEEDED','TRANSIENT_FAILURE','RATE_LIMIT','AUTH_REQUIRED','INVALID_CONTENT','UNKNOWN','FAILED_FINAL') then
    raise exception 'invalid worker outcome' using errcode = '22023';
  end if;
  ...
  if p_outcome = 'SUCCEEDED' then
  ...
  elsif p_outcome in ('TRANSIENT_FAILURE','RATE_LIMIT')
    ...
    update public.post_targets
       set state = 'NEEDS_ACTION', updated_at = now()
     where id = v_job.post_target_id
       and organization_id = v_job.organization_id;
    ...
    update public.scheduled_jobs
       set status = 'SUCCEEDED', locked_until = null,
           last_error_code = coalesce(p_error_code, 'UNKNOWN_OUTCOME'),
           updated_at = now()
     where id = v_job.id;
    ...
    values (
      v_job.organization_id, v_job.post_target_id,
      'RECONCILE_TARGET', now() + interval '2 minutes', 'READY',
      0, 3,
      'reconcile:' || v_job.post_target_id::text || ':' || v_job.attempts::text,
      jsonb_build_object('source_job_id', v_job.id, 'reason', 'unknown_publish_outcome')
    )
    ...
    update public.post_targets
       set state = 'FAILED_FINAL', updated_at = now()
     where id = v_job.post_target_id
       and organization_id = v_job.organization_id;
...
revoke all on function public.worker_finish_publication_job(uuid,text,text,text,integer,text,integer) from public, anon, authenticated;
grant execute on function public.worker_finish_publication_job(uuid,text,text,text,integer,text,integer) to service_role;
