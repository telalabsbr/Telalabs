...
const weekSlots = [
  { day: 0, time: "09:00", title: "Rotina de cuidados", networks: ["instagram","facebook"] as const, status: "scheduled" as const },
  { day: 1, time: "11:00", title: "Novo procedimento", networks: ["tiktok","instagram"] as const, status: "scheduled" as const },
  { day: 2, time: "09:30", title: "Skin care matinal", networks: ["instagram","tiktok","linkedin"] as const, status: "scheduled" as const },
  { day: 2, time: "12:00", title: "Reels: Resultado real", networks: ["instagram","facebook","youtube"] as const, status: "scheduled" as const, selected: true },
  { day: 2, time: "16:30", title: "Antes e depois", networks: ["instagram","youtube"] as const, status: "scheduled" as const },
  { day: 3, time: "13:00", title: "Perguntas frequentes", networks: ["tiktok","linkedin"] as const, status: "scheduled" as const },
  { day: 4, time: "12:30", title: "Promoção do mês", networks: ["instagram","facebook"] as const, status: "scheduled" as const },
  { day: 4, time: "18:00", title: "Depoimento em vídeo", networks: ["youtube","instagram"] as const, status: "scheduled" as const },
  { day: 5, time: "16:00", title: "Estética e autoestima", networks: ["linkedin"] as const, status: "scheduled" as const },
  { day: 6, time: "11:00", title: "Domingo relax", networks: ["instagram"] as const, status: "scheduled" as const },
];
  ...
  return <div className="space-y-4">
    <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Organize a operação sem perder o contexto de cada publicação.</p>
      </div>
    ...
    <section className="card overflow-hidden">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex rounded-lg bg-slate-100 p-1">
              {([["month","Mês"],["week","Semana"],["list","Lista"]] as const).map(([value,label]) => <button key={value} onClick={() => setView(value)} className={`rounded-md px-3 py-2 text-xs font-bold ${view === value ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>{label}</button>)}
            </div>
            <button className="btn-secondary !min-h-9 !px-2.5" aria-label="Período anterior"><ChevronLeft size={16}/></button>
            <div className="min-w-48 text-center text-sm font-black text-slate-900">22 – 28 de setembro de 2026</div>
            <button className="btn-secondary !min-h-9 !px-2.5" aria-label="Próximo período"><ChevronRight size={16}/></button>
            <button className="btn-secondary !min-h-9">Hoje</button>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button className="btn-secondary !min-h-9"><SlidersHorizontal size={15}/> Todas as redes</button>
            <button className="btn-secondary !min-h-9">Todos os status</button>
            <label className="relative min-w-56 flex-1 xl:flex-none">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={15}/>
              <input className="field !min-h-9 pl-9 pr-3 text-xs" placeholder="Buscar no calendário..."/>
            </label>
          </div>
        </div>
      </div>

      {view === "week" && <div className="app-scrollbar overflow-x-auto">
      ...
      {view === "month" && <div>
        <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">{["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"].map(day => <div key={day} className="p-3 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500">{day}</div>)}</div>
        <div className="grid grid-cols-7">{monthDays.map((day,index) => <div key={index} className={`min-h-28 border-b border-r border-slate-100 p-2 ${index === 24 ? "bg-indigo-50/40" : "bg-white"}`}>
          <span className="text-xs font-bold text-slate-700">{day}</span>
          {[3,8,10,15,17,22,24,29].includes(day) && <button onClick={() => setDrawerOpen(true)} className="mt-2 w-full rounded-lg bg-indigo-50 p-2 text-left text-[10px] font-bold text-indigo-800">2 publicações</button>}
        </div>)}</div>
      </div>}

      {view === "list" && <div className="divide-y divide-slate-100">
        {weekSlots.map(item => <button key={item.title} onClick={() => setDrawerOpen(true)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-slate-50">
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-indigo-50 text-xs font-black text-indigo-700">{weekDays[item.day].day}</div>
          ...
          <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-bold text-amber-700">Agendado</span>
        </button>)}
      </div>}

      <div className="flex flex-wrap items-center gap-3 border-t border-slate-200 px-4 py-3 text-[11px] text-slate-500">
        ...
        <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-amber-500"/> Com aviso</span>
        <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-red-500"/> Erro</span>
        <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-slate-400"/> Rascunho</span>
        <span className="ml-auto text-slate-400">Drag-and-drop entra junto da persistência real.</span>
      </div>
    </section>

    {drawerOpen && <aside className="fixed inset-y-16 right-0 z-30 hidden w-[380px] overflow-y-auto border-l border-slate-200 bg-white shadow-2xl xl:block">
      <div className="flex items-center justify-between border-b border-slate-200 p-4">
        <div><p className="text-sm font-black text-slate-950">Detalhes da publicação</p><p className="mt-0.5 text-xs text-slate-500">Quarta, 24 set · 12:00</p></div>
        <button onClick={() => setDrawerOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><X size={18}/></button>
      </div>
      <div className="space-y-5 p-4">
        ...
        <div>
          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Legenda</p>
        ...
        <div>
          ...
          <div className="mt-2 divide-y divide-slate-100 rounded-xl border border-slate-200">
            {selectedPublication.destinations.map(destination => <div key={destination.id} className="flex items-center gap-3 p-3">
              ...
              <div className="min-w-0 flex-1"><p className="text-xs font-bold text-slate-900">{platformLabels[destination.platform]}</p><p className="text-[11px] text-slate-500">24/09 · 12:00</p></div>
