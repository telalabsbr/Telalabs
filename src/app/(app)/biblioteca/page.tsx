...
export default function LibraryPage() {
  ...
  const [selected, setSelected] = useState<MediaItem | null>(items[0]);
  ...
  return <div className="space-y-4">
      ...
      <div className="border-b border-slate-200 p-4">
        <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
          <label className="relative">
            ...
            <input value={query} onChange={event => setQuery(event.target.value)} className="field pl-9 pr-3 text-sm" placeholder="Buscar na biblioteca..."/>
          </label>
          <div className="flex items-center gap-3">
            <div className="min-w-52">
              <div className="flex items-center justify-between text-[11px] font-semibold text-slate-500"><span>7,2 GB de 20 GB</span><span>36%</span></div>
        ...
        <div className="app-scrollbar mt-3 flex gap-2 overflow-x-auto">
          {([
            ["all","Todos"],
            ["video","Vídeos"],
            ["image","Imagens"],
            ["favorite","Favoritos"],
          ] as const).map(([value,label]) => <button key={value} onClick={() => setFilter(value)} className={`shrink-0 rounded-lg border px-3 py-2 text-xs font-bold ${filter === value ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-600"}`}>{label}</button>)}
        </div>
      </div>

      <div className="p-4">
        {view === "grid" ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
          {visible.map(item => <button key={item.id} onClick={() => setSelected(item)} className={`overflow-hidden rounded-xl border bg-white text-left transition hover:-translate-y-0.5 hover:shadow-md ${selected?.id === item.id ? "border-indigo-400 ring-2 ring-indigo-100" : "border-slate-200"}`}>
            <div className={`relative aspect-video bg-gradient-to-br ${item.tone}`}>
              ...
              {item.duration && <span className="absolute bottom-2 left-2 rounded-md bg-slate-950/75 px-1.5 py-1 text-[10px] font-bold text-white">{item.duration}</span>}
            ...
            <div className="flex items-start gap-2 p-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11px] text-slate-500">{item.date} · {item.size}</p>
              </div>
          ...
          {visible.map(item => <button key={item.id} onClick={() => setSelected(item)} className="flex w-full items-center gap-3 p-3 text-left hover:bg-slate-50">
            ...
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{item.name}</p><p className="mt-0.5 text-xs text-slate-500">{item.date} · {item.size}</p></div>
            {item.favorite && <Heart className="fill-amber-400 text-amber-500" size={16}/>}
    ...
    {selected && <aside className="fixed inset-y-16 right-0 z-30 hidden w-[360px] overflow-y-auto border-l border-slate-200 bg-white shadow-2xl xl:block">
